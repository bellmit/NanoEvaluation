package com.nano.msc.auth.service.impl;
...
import com.nano.msc.auth.repository.AuthPermissionRepository;
import com.nano.msc.auth.repository.AuthRolePermissionRepository;
import com.nano.msc.auth.repository.AuthUserRepository;
...
import com.nano.msc.auth.util.JwtTokenUtil;
...
public class AuthUserServiceImpl implements AuthUserService {
    ...
    public AuthUser register(AuthUser user) {
        if (getUserByUsername(user.getUsername()) != null) {
            ExceptionAsserts.fail("该用户名已经注册");
        }
        user.setStatus(1);
        // 将密码进行加密操作
