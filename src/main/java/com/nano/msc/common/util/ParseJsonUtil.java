...
public class ParseJsonUtil {
     ...
     * @throws ParseException 返回错误信息
     */
    public static <T> T parseAndCheck(String jsonData, Class<T> tClass) throws ParseException {
        T t = null;
        try {
            t = ParseJsonUtil.getObject(tClass, jsonData);
        } catch (JsonSyntaxException j) {
            ParseException.dataFormatException(j, jsonData);
        }

        // 进行数据校验
        Map<String, String> validResult = ValidatorUtils.validate(t);
        if (Objects.nonNull(validResult) && !validResult.isEmpty()) {
            ParseException.dataFormatException(validResult, jsonData);
        }
