package com.nano.msc.evaluation.utils;
...
public class ServiceCrudCheckUtils {
    ...
    private static CommonResult checkSaveSuccess(Object result, Object saveObject) {
        if (Objects.isNull(result)) {
            ...
            return CommonResult.failed(ResultVo.error(ExceptionEnum.UNKNOWN_ERROR, saveObject));
        }
    ...
    public static <T, ID> CommonResult saveObjectAndCheck(JpaRepository<T, ID> jpaRepository, T t) {
        ...
        if (Objects.isNull(jpaRepository)) {
            return CommonResult.failed(ResultVo.error(ExceptionEnum.UNKNOWN_ERROR, t));

        }
        Map<String, String> validResult = ValidatorUtils.validate(t);
        if (Objects.nonNull(validResult) && validResult.size() != 0) {
            return CommonResult.failed(ResultVo.error(ExceptionEnum.DATA_FORMAT_ERROR, validResult));
        }

        if (Objects.nonNull(getSearchObjectFromDataBase(jpaRepository, t))) {
            logError("object is existed, object is : " + t.toString());

            return CommonResult.failed(ResultVo.error(ExceptionEnum.DATA_EXISTED, t.getClass().getSimpleName() + " object is existed"));
        }
        T result;
        try {
            ...
            logError(e);
            return CommonResult.failed(ResultVo.error(ExceptionEnum.DATA_FORMAT_ERROR, e.getCause().toString() + "\n" + t));
        } catch (Exception e) {
            logError(e);
            return CommonResult.failed(ResultVo.error(ExceptionEnum.UNKNOWN_ERROR, e.getCause().toString()+ "" +
                    "\n" + t));
        }
...
//    }
    ...
    public static <T, ID> CommonResult updateObjectAndCheck(JpaRepository<T, ID> jpaRepository, T t) {
        ...
        if (Objects.isNull(searchObject)) {
            return CommonResult.failed(ResultVo.error(ExceptionEnum.DATA_EXISTED, t));
        }
        ...
        try {
            ...
            // 获取到校验错误，即数据字段可能为空或错误
            logError(e);
            return CommonResult.failed(ResultVo.error(ExceptionEnum.DATA_FORMAT_ERROR, e.toString() + "\n" + t));
        } catch (Exception e) {
            logError(e);
            return CommonResult.failed(ResultVo.error(ExceptionEnum.UNKNOWN_ERROR, e.toString() + "\n" + t));
        }
    ...
    public static <T, ID> CommonResult deleteObjectByIdAndCheck(JpaRepository<T, ID> jpaRepository, ID id) {
        if (Objects.isNull(jpaRepository)) {
            return CommonResult.failed(ResultVo.error(ExceptionEnum.UNKNOWN_ERROR, id));
        }
        ...
        if (Objects.nonNull(id)) {
            ...
            if (!existFlag) {
                ...
                return CommonResult.failed(ResultVo.error(ExceptionEnum.DATA_NOT_EXIST));
            }
            // 尝试通过id删除
            try {
